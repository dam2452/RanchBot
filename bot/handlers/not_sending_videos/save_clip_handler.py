...
class SaveClipHandler(BotMessageHandler):
    ...
    async def __check_clip_name_unique(self) -> bool:
        clip_name = self._message.get_text().split(maxsplit=1)[1]
        if not await db.is_clip_name_unique(self._message.get_chat_id(), clip_name):
            await self.__reply_clip_name_exists(clip_name)
            return False
        return True

    async def __check_clip_limit_not_exceeded(self) -> bool:
        is_admin_or_moderator = await db.is_admin_or_moderator(self._message.get_user_id())
        user_clip_count = await db.get_user_clip_count(self._message.get_chat_id())
    ...
    async def __check_last_clip_exists(self) -> bool:
        last_clip = await db.get_last_clip_by_chat_id(self._message.get_chat_id())
        if not last_clip:
