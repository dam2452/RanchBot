...
class SerialContextManager:
    ...
    async def get_user_active_series(self, user_id: int) -> str:
        series_id = await db.get_user_active_series(user_id)
        if series_id:
            series_name = await db.get_series_by_id(series_id)
            if series_name:
                return series_name
        return settings.DEFAULT_SERIES

    async def set_user_active_series(self, user_id: int, series_name: str) -> None:
        series_id = await db.get_or_create_series(series_name)
        await db.set_user_active_series(user_id, series_id)
