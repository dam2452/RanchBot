...
class TestAdjustVideoClipHandler(BaseTest):
    ...
    async def test_invalid_args_count(self):
        video_name = "geniusz"
        self.assert_command_result_file_matches(await self.send_command(f"/klip {video_name}"), f"clip_{video_name}.mp4")
    ...
    async def test_adjust_clip_with_valid_params(self):
        ...
        self.assert_command_result_file_matches(await self.send_command(f"/klip {video_name}"), "clip_geniusz.mp4")

        self.assert_command_result_file_matches(await self.send_command(f"/d {adjust_params}"), adjusted_filename)
        self.assert_command_result_file_matches(await self.send_command(f"/ad {adjust_params}"), adjusted_filename)
    ...
    async def test_consecutive_relative_adjustments(self):
        ...
        self.assert_command_result_file_matches(await self.send_command(f"/d {adjust_params}"), adjusted_filename)
        self.assert_command_result_file_matches(await self.send_command(f"/d {adjust_params}"), adjusted_filename_2)
