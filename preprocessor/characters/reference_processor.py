...
class CharacterReferenceProcessor(BaseProcessor):
        ...
        for first_face in first_image_faces:
            ...
            for other_image_faces in remaining_images:
                ...
                for other_face in other_image_faces:
                    similarity = np.dot(first_face.face_vector, other_face.face_vector)
                    if similarity > best_similarity:
                        best_similarity = similarity  # pylint: disable=redefined-variable-type
                ...
                if best_match:
                    ...
                    if best_similarity < self.similarity_threshold:
                        console.print(f"[yellow]Warning: Low similarity {best_similarity:.2f} < {self.similarity_threshold:.2f}[/yellow]")
                else:
                    break
    ...
    def _ask_user_to_select_candidate(
        ...
        while True:
            user_input = input(f"Select the correct character (1-{len(candidates)}) or skip (s): ").strip().lower()  # pylint: disable=bad-builtin
    ...
    def _ask_user_to_select_initial_face(
        ...
        while True:
            user_input = input(f"Select the correct face (1-{len(first_image_faces)}) or skip (s): ").strip().lower()  # pylint: disable=bad-builtin
            ...
            try:
                ...
                console.print(f"[red]Invalid selection. Please enter 1-{len(first_image_faces)} or 's'[/red]")
            except ValueError:
                console.print("[red]Invalid input. Please enter a number or 's'[/red]")
    ...
    def _find_matching_faces_for_reference(
        ...
        for img_idx, other_image_faces in enumerate(remaining_images, 1):
            if not other_image_faces:
                console.print(
                    f"[red]No faces found in image {img_idx + 1}: {reference_images[img_idx]}[/red]",
                )
                return None
            ...
            for other_face in other_image_faces:
                similarity = np.dot(reference_vector, other_face.face_vector)
                if similarity > best_sim:
                    best_sim = similarity  # pylint: disable=redefined-variable-type
                    best_match = other_face
            ...
            if best_match:
                ...
                if best_sim < self.similarity_threshold:
                    console.print(
                        f"[yellow]Warning: Low similarity in image {img_idx + 1}: {reference_images[img_idx]} "
                        f"(similarity: {best_sim:.2f} < threshold: {self.similarity_threshold:.2f})[/yellow]",
                    )
            else:
                console.print(
                    f"[red]No faces detected in image {img_idx + 1}: {reference_images[img_idx]}[/red]",
                )
                return None
    ...
    def generate_validation_grid(self) -> None:  # pylint: disable=too-many-locals,too-many-statements
        ...
        for idx, char_dir in enumerate(processed_chars):
            ...
            cv2.rectangle(
                grid,
                (0, y_offset - padding),
                (grid_width, y_offset + face_size + padding),
                row_bg,
                -1,
            )
            ...
            cv2.putText(
                grid,
                char_name,
                (padding * 2, y_offset + face_size // 2),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.55,
                (30, 40, 50),
                1,
        ...
        footer_text = f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} | " \
                     ...
                     f"Normalized Size: {settings.character.normalized_face_size[0]}x{settings.character.normalized_face_size[1]}px"
