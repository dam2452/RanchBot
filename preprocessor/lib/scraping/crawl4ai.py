...
class ScraperCrawl4AI:
    ...
    @staticmethod
    def scrape(url: str, save_markdown: bool=False, output_dir: Optional[Path]=None, logger: Optional[ErrorHandlingLogger]=None) -> Optional[str]:
    ...
    @staticmethod
    def __save_markdown(content: str, url: str, output_dir: Path, logger: Optional[ErrorHandlingLogger]=None) -> None:
    ...
    @staticmethod
    async def __scrape_async(url: str, save_markdown: bool=False, output_dir: Optional[Path]=None, logger: Optional[ErrorHandlingLogger]=None) -> Optional[str]:
        try:
            ua = ua_generator.generate()
            browser_config = BrowserConfig(headless=True, enable_stealth=True, viewport_width=1920, viewport_height=1080, user_agent=str(ua))
            ...
            async with AsyncWebCrawler(config=browser_config) as crawler:
                ...
                if result.success:
                    if save_markdown and output_dir:
                        ScraperCrawl4AI.__save_markdown(result.markdown, url, output_dir, logger)
