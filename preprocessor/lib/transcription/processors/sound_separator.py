...
class SoundEventSeparator(BaseProcessor):
    ...
    def __init__(self, args: Dict[str, Any]) -> None:
        super().__init__(args=args, class_name=self.__class__.__name__, error_exit_code=2, loglevel=args.get('loglevel', 20))
        self.transcription_dir = Path(self._args.get('transcription_dir', settings.transcription.get_output_dir(self.series_name)))
    ...
    def _process_item(  # pylint: disable=too-many-locals
        ...
        with open(clean_json, 'w', encoding='utf-8') as f:
            json.dump({'episode_info': episode_info, 'segments': dialogue_segments_simple}, f, ensure_ascii=False, indent=4)
        with open(sound_json, 'w', encoding='utf-8') as f:
            json.dump({'episode_info': episode_info, 'segments': sound_event_segments_simple}, f, ensure_ascii=False, indent=4)
        with open(clean_segmented_json, 'w', encoding='utf-8') as f:
            json.dump({'episode_info': episode_info, 'segments': dialogue_segments}, f, ensure_ascii=False, indent=4)
        with open(sound_segmented_json, 'w', encoding='utf-8') as f:
            json.dump({'episode_info': episode_info, 'segments': sound_event_segments}, f, ensure_ascii=False, indent=4)
    ...
    def __finalize_sequence(
        ...
        new_segment = {'text': text, 'start': start_time, 'end': end_time, 'words': words}
