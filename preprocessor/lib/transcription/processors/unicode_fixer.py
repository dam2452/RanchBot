...
from preprocessor.lib.transcription.utils import TranscriptionUtils
...
class TranscriptionUnicodeFixer(BaseProcessor):
    ...
    def __init__(self, args: Dict[str, Any]) -> None:
        ...
        default_dir = settings.transcription.get_output_dir(self.series_name)
        self.transcription_jsons = Path(
            self._args.get('transcription_jsons', default_dir),
        )
    ...
    def _process_item(self, item: ProcessingItem, missing_outputs: List[OutputSpec]) -> None:
        ...
        try:
            was_fixed = TranscriptionUtils.fix_transcription_file_unicode(trans_file)
            if was_fixed:
