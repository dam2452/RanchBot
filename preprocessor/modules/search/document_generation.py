...
class DocumentGeneratorStep(PipelineStep[Artifact, ElasticDocuments, DocumentGenerationConfig]):
    ...
    def execute(self, input_data: Artifact, context: ExecutionContext) -> ElasticDocuments:
        ...
        data = self._gather_input_data(episode_info, context)
        generated_files = []
        total_docs = 0
        if self.config.generate_segments and 'transcription' in data:
            path, count = self._generate_segments_jsonl(data, episode_info, context)
    ...
    @staticmethod
    def _gather_input_data(episode_info: Any, context: ExecutionContext) -> Dict[str, Any]:
        data = {}
    ...
    def _generate_segments_jsonl(self, data: Dict[str, Any], episode_info: Any, context: ExecutionContext) -> tuple[Path, int]:
        ...
        episode_metadata = self._build_episode_metadata(episode_info, context)
    ...
    @staticmethod
    def _build_episode_metadata(episode_info: Any, context: ExecutionContext) -> Dict[str, Any]:
