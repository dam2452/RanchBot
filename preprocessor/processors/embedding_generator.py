...
@register_processor("generate_embeddings")
class EmbeddingGenerator(BaseProcessor): # pylint: disable=too-many-instance-attributes
    REQUIRES = ["transcriptions", "frames"]
    PRODUCES = ["embeddings"]
    PRIORITY = 50
    DESCRIPTION = "Generate multimodal embeddings"
    ...
    def get_output_subdir(self) -> str:
        return settings.output_subdirs.embeddings
    ...
    def _get_processing_items(self) -> List[ProcessingItem]:
        ...
        for trans_file in all_transcription_files:
            ...
            if trans_file.parent.name in {"clean", "sound_events"}:
                continue
    ...
    def _get_expected_outputs(self, item: ProcessingItem) -> List[OutputSpec]:
        ...
        if self.generate_text:
            ...
            text_output = self._build_output_path(episode_info, text_filename)
        ...
        if self.generate_episode_names:
            ...
            episode_name_output = self._build_output_path(episode_info, episode_name_filename)
        ...
        if self.generate_video:
            ...
            video_output = self._build_output_path(episode_info, video_filename)
        ...
        if self.generate_full_episode:
            ...
            full_episode_output = self._build_output_path(episode_info, full_episode_filename)
        ...
        if self.generate_sound_events:
            ...
            sound_events_output = self._build_output_path(episode_info, sound_events_filename)
    ...
    def _process_item(self, item: ProcessingItem, missing_outputs: List[OutputSpec]) -> None:  # pylint: disable=too-many-locals,too-many-statements
        ...
        if not clean_transcription_file.exists():
            self.logger.warning(f"Clean transcription not found: {clean_transcription_file}, skipping text embeddings generation")
    ...
    def __generate_sound_event_embeddings(self, trans_file: Path) -> List[Dict[str, Any]]:  # pylint: disable=too-many-locals
        ...
        if not sound_events_file.exists():
            self.logger.warning(f"Sound events file not found: {sound_events_file}, skipping sound event embeddings generation")
            return []
    ...
    def __generate_full_episode_embedding(self, trans_file: Path) -> Optional[Dict[str, Any]]:  # pylint: disable=too-many-locals,too-many-statements
        ...
        if not clean_txt_file.exists():
            self.logger.warning(f"Clean transcript file not found: {clean_txt_file}")
            return None

        try: # pylint: disable=too-many-try-statements
            with open(clean_txt_file, "r", encoding="utf-8") as f:
                full_text = f.read().strip()

            if not full_text:
                self.logger.warning(f"Empty clean transcript file: {clean_txt_file}")
                return None
    ...
    def __load_frame_metadata(self, episode_info_dict: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        frames_episode_dir = self.path_manager.get_episode_dir(episode_info_obj, settings.output_subdirs.frames)
    ...
    def __load_image_hashes(self, episode_info_dict: Dict[str, Any]) -> Dict[int, str]:
        return load_image_hashes_for_episode(episode_info_dict, self.series_name, self.logger)
    ...
    def __generate_video_embeddings(self, episode_info_dict: Dict[str, Any], frame_metadata: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        frames_episode_dir = self.path_manager.get_episode_dir(episode_info_obj, settings.output_subdirs.frames)
        episode_output_dir = self.path_manager.get_episode_dir(episode_info_obj, settings.output_subdirs.embeddings)
    ...
    def __get_episode_output_dir(self, transcription_file: Path) -> Path:
        ...
        if episode_info_from_file:
            return self.path_manager.get_episode_dir(episode_info_from_file, settings.output_subdirs.embeddings)
        return self.path_manager.base_output_dir / settings.output_subdirs.embeddings
