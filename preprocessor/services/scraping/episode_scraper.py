...
class EpisodeScraper(BaseScraper):
    ...
    def __validate_and_report_coverage(self, scraped_count: int) -> None:
        expected_count = self.__get_expected_episodes_count()
        if expected_count is None:
            self.__print_no_validation_warning(scraped_count)
            return

        status, message = self.__evaluate_coverage_status(scraped_count, expected_count)
        self.__print_coverage_report(scraped_count, expected_count, status, message)

    def __get_expected_episodes_count(self) -> Optional[int]:
        if self.__expected_episodes_count is not None:
            return self.__expected_episodes_count
        if self.__videos_dir and self.__videos_dir.exists():
            return self.__count_video_files(self.__videos_dir)
        return None
    ...
    @staticmethod
    def __evaluate_coverage_status(scraped: int, expected: int) -> Tuple[str, str]:
        if scraped < expected:
