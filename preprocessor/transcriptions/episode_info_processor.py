...
class EpisodeInfoProcessor:
    def __init__(
        ...
        if not series_name:
            ...
            self.__logger.warning(
                f"No series name provided. Using fallback from folder name: '{series_name}'",
            )
    ...
    @staticmethod
    def __load_transcription(path: Path) -> dict:
        with path.open("r", encoding="utf-8") as f:
            return json.load(f)

    def __write_episode_json(self, transcription: dict, episode_info: dict) -> Tuple[Path, str]:
