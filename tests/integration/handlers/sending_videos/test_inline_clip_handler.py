...
class TestInlineClipHandlerIntegration(BaseIntegrationTest):
    ...
    @pytest.mark.asyncio
    async def test_inline_clip_with_saved_clip(self, mock_db, mock_ffmpeg):
    ...
    @pytest.mark.asyncio
    async def test_inline_clip_with_search_results(self, mock_es, mock_ffmpeg):
    ...
    @pytest.mark.asyncio
    async def test_inline_clip_no_results(self):
    ...
    @pytest.mark.asyncio
    async def test_inline_clip_missing_argument(self):
    ...
    @pytest.mark.asyncio
    async def test_inline_clip_with_multiple_words(self, mock_es, mock_ffmpeg):
        segment = TestSegments.get_geniusz_segment()
        ep_info = segment.pop('episode_info')
        segment.pop('_quote_keywords', None)
        mock_es.add_segment(
